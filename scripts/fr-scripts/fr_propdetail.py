...
for row in cursor:
	...
	pagename = str(row['intbeds'])+'-chambre-'+fr_proptype.replace(' ','-').replace('é','e')+'-a-'+row['strlocation_detail'].replace(' ','-')+'-'+row['strpropertyid']
	...
	propdict = {}
	propdict['props'] = []
	#propdict['propdescription'] = removeumlauts(row['strdescription_FR'])
	propdict['propdescription'] = row['strdescription_FR']
